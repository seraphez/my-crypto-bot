...
if "cached_portfolio_analysis" not in st.session_state:
    st.session_state.cached_portfolio_analysis = ""
if "previous_favs" not in st.session_state:
    st.session_state.previous_favs = []
...
def get_rough_direction(change_pct):
    ...
    elif 5.0 <= change_pct < 10.0: return "🟢 ［多頭強勢突破］ 成交量同步放大，健康上攻趨勢", "bull"
    elif change_pct <= -10.0: return "⚠️ ［全網恐慌砸盤］ 鏈式爆倉觸發，切勿盲目接刀抄底", "bear"
    elif -10.0 < change_pct <= -5.0: return "🔴 ［空頭大單壓制］ 空方資金淨流出，下行慣性強烈", "bear"
    return "⚪ ［常態籌碼沉澱］ 無明顯方向，維持窄幅洗盤", "neutral"

# 🎯 全新：Gemini 自選組合「多幣連通資金流向」深度分析引擎
...
# 自選組合變更時，自動清空快取讓 AI 重新聯通分析
if fav_cryptos != st.session_state.previous_favs:
    st.session_state.cached_portfolio_analysis = ""
    st.session_state.previous_favs = fav_cryptos
...
# =====================================================================
# 7. 主畫面雙欄自適應佈局 (資料解耦、各自歸位)
# =====================================================================
st.title("🏹 CryptoHunter 智能雷達")
st.markdown("---")

# ---------------------------------------------------------------------
# 模式 A：📊 自選戰研與 AI 建議 (左自選、右爆量提醒)
# ---------------------------------------------------------------------
if page_view == "📊 自選戰研與 AI 建議":
    ...
    with col_left:
        ...
        if fav_data_list:
            ...
            if not st.session_state.cached_portfolio_analysis:
                with st.spinner("正在將自選幣進行跨市場資金連通性調研..."):
                    st.session_state.cached_portfolio_analysis = ask_gemini_portfolio_analysis(fav_data_list)
